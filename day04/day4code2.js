...
function loopdiloop(bingoSheetsAll, callSequence) {
		...
		for(let i = 0; i < bingoSheetsAll.length; i++) {
			for (let j = 0; j < bingoSheetsAll[i].length; j++) {
				for(let k = 0; k < bingoSheetsAll[i][j].length; k++) {
					...
					if(finishedBoardsIndices.includes(i) == false) {


						if (bingoSheetsAll[i][j][k] === calls) {
							bingoSheetsAll[i][j][k] = 'x'; 
							if (checkVertical(bingoSheetsAll[i])) {
								finishedBoardsIndices.push(i)
								madeCalls.push(calls)
	...
	let lastBoardIndex = finishedBoardsIndices[finishedBoardsIndices.length-1];
	let lastCall = madeCalls[madeCalls.length-1];
	//console.log(bingoSheetsAll[lastBoardIndex], lastCall);
	console.log('BINGO!!!');
	console.log('The last number called is ', lastCall)
	console.log('The last winner is board ', lastBoardIndex); 
	console.log(bingoSheetsAll[lastBoardIndex]);
	let sum = sumUpNonX(bingoSheetsAll[lastBoardIndex]);
	console.log('The end score is ', lastCall*sum);
